...
import { competitionCategories } from "@/data/competition-categories";
...
export function ProjectBrowser({ projects }: { projects: Project[] }) {
  ...
  const options = useMemo(() => ({
    ...
    competitions: [...competitionCategories],
  ...
  const filtered = useMemo(() => {
    ...
    const result = projects.filter((project) => {
      ...
      return (!q || searchable.includes(q)) &&
        ...
        (!value("competition") || project.competitionCategories.includes(value("competition") as (typeof project.competitionCategories)[number])) &&
    ...
    <div className="mt-8">
      <div className="rounded-3xl border border-line bg-surface p-4 shadow-soft sm:p-5">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Filter label="競賽" value={value("competition")} items={numberedOptions(options.competitions, false)} onChange={(v) => update("competition", v)} className={`${selectClass} bg-accent/15`} />
          <Filter label="技術" value={value("technology")} items={numberedOptions(options.technologies)} onChange={(v) => update("technology", v)} className={`${selectClass} bg-primary/15`} />
          <Filter label="議題" value={value("topic")} items={numberedOptions(options.topics)} onChange={(v) => update("topic", v)} className={`${selectClass} bg-secondary/30`} />
...
const numberedOptions = (items: readonly string[], addNumber = true): FilterOption[] =>
  items.map((item, index) => ({ value: item, label: addNumber ? `${index + 1}. ${item}` : item }));

function Filter({ label, value, items, onChange, className }: { label: string; value: string; items: readonly (string | FilterOption)[]; onChange: (value: string) => void; className: string }) {
  return <label><span className="sr-only">{label}篩選</span><select value={value} onChange={(event) => onChange(event.target.value)} className={`${className} w-full`}><option value="">所有{label}</option>{items.map((item) => { const option = typeof item === "string" ? { value: item, label: item } : item; return <option key={option.value} value={option.value}>{option.label}</option>; })}</select></label>;
}
